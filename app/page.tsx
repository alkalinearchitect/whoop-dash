...
import type { WhoopCycle, WhoopSleep, WhoopWorkout, WhoopProfile } from "@/lib/whoop";
import { generateMockData, type MockDashboardData } from "@/lib/whoop";
...
function getTrendDir(curr: number, prev: number, higherBetter: boolean): "up" | "down" | "neutral" {
  const diff = curr - prev;
  if (Math.abs(diff) < 1) return "neutral";
  return (diff > 0) === higherBetter ? "up" : "down";
}
...
export default function WhoopDashboard() {
  ...
  const recoveryTrend = trendCycles.map((c) => ({
    date: c.start,
    value: c.score?.recovery_score != null ? Math.round(c.score.recovery_score * (c.score.recovery_score <= 1 ? 100 : 1)) : 0,
  }));
  ...
  const sleepTrend = (data?.sleep ?? []).slice(0, 30).reverse().map((s) => ({
    date: s.start,
    value: s.score?.sleep_performance_percentage != null ? Math.round(s.score.sleep_performance_percentage * (s.score.sleep_performance_percentage <= 1 ? 100 : 1)) : 0,
  }));
    ...
    <div className="app-shell">
      ...
      <div className="flex items-center justify-between mb-5 mt-2">
        ...
        <div className="flex items-center gap-2">
          {data?.source === "mock" && <span className="pill pill--warn">Demo</span>}
        </div>
      </div>
      ...
      {tab === "overview" && (
        <div className="space-y-3">
          <RecoveryGauge
            score={recoveryScore}
            explanation="How ready your body is for strain today. Green (66-100%) = go hard. Amber (33-65%) = moderate effort. Red (0-32%) = rest day."
            loading={loading}
          />
          <div className="card-grid--2col">
            <MetricCard value={sleepPerf != null ? `${sleepPerf}%` : "—"} label="Sleep Performance" explanation="How much of your sleep need was actually met last night." loading={loading} size="md" />
            <MetricCard value={todayStrain != null ? todayStrain.toFixed(1) : "—"} label="Today's Strain" explanation="How hard you've pushed today on a 0-21+ scale." loading={loading} size="md" />
          </div>
          <div className="card-grid--2col">
            <MetricCard value={hrv ?? "—"} label="HRV" unit="ms" explanation="Heart Rate Variability. Higher = more recovered, less stress." loading={loading} size="sm" />
            <MetricCard value={rhr ?? "—"} label="Resting HR" unit="bpm" explanation="Your heart rate at complete rest. Lower = better fitness." loading={loading} size="sm" />
            <MetricCard value={respRate ?? "—"} label="Respiratory Rate" unit="br/min" explanation="Breaths per minute during sleep. Normal is 12-20." loading={loading} size="sm" />
            <MetricCard value={spo2 ? `${spo2}%` : "—"} label="SpO2" explanation="Blood oxygen saturation. Normal is 95-100%." loading={loading} size="sm" />
          </div>
          <MetricCard value={skinTemp ? `${skinTemp}°C` : "—"} label="Skin Temperature" explanation="Your skin temperature during sleep. Tracked as deviation from your baseline." loading={loading} size="md" />
          <div className="ai-readout">
            <p className="text-[11px] text-white/70 leading-relaxed">
              {loading ? "Analyzing your data..." : (
                ...
                recoveryScore >= 66 ? "🟢 You're fully recovered. Today is a great day for a hard session." :
                recoveryScore >= 33 ? "🟡 Moderate recovery. A solid workout is fine, but stay in zones 2-4." :
                "🔴 Low recovery. Light movement only — walk, stretch, or rest."
              )}
            </p>
          </div>
        </div>
      ...
      {tab === "sleep" && (
        <div className="space-y-3">
          <SleepScoreChart sleep={latestSleep} explanation="Your sleep score breaks down REM, light, and deep sleep." loading={loading} />
          <div className="card-grid--2col">
            <MetricCard value={sleepConsistency != null ? `${sleepConsistency}%` : "—"} label="Sleep Consistency" explanation="How consistent your sleep/wake times have been." loading={loading} size="md" />
            <MetricCard value={latestSleep?.score?.sleep_efficiency_percentage != null ? `${Math.round(latestSleep.score.sleep_efficiency_percentage * (latestSleep.score.sleep_efficiency_percentage <= 1 ? 100 : 1))}%` : "—"} label="Sleep Efficiency" explanation="Percentage of time in bed that you were actually asleep." loading={loading} size="md" />
          </div>
          {latestSleep?.score?.sleep_needed && (
            <div className="glass p-4">
              <p className="section-title">Sleep Need Breakdown</p>
              <div className="card-grid--2col mt-3">
                <MetricCard value={fmtTime(latestSleep.score.sleep_needed.baseline_milli)} label="Baseline Need" explanation="Your average sleep need." loading={loading} size="sm" />
                <MetricCard value={fmtTime(latestSleep.score.sleep_needed.need_from_sleep_debt_milli)} label="Sleep Debt" explanation="Extra sleep to pay back recent short nights." loading={loading} size="sm" />
                <MetricCard value={fmtTime(latestSleep.score.sleep_needed.need_from_recent_strain_milli)} label="Strain Recovery" explanation="Extra sleep to recover from hard efforts." loading={loading} size="sm" />
                <MetricCard value={fmtTime(latestSleep.score.sleep_needed.baseline_milli + latestSleep.score.sleep_needed.need_from_sleep_debt_milli + latestSleep.score.sleep_needed.need_from_recent_strain_milli + latestSleep.score.sleep_needed.need_from_recent_nap_milli)} label="Total Need" explanation="Total sleep your body needed last night." loading={loading} size="sm" />
              </div>
            </div>
          )}
      ...
      {tab === "strain" && (
        <div className="space-y-3">
          <div className="card-grid--2col">
            <MetricCard value={todayStrain != null ? todayStrain.toFixed(1) : "—"} label="Today's Strain" explanation="How hard you've pushed today on a 0-21+ scale." loading={loading} size="md" />
            <MetricCard value={latestCycle?.score?.heart_rate_avg ?? "—"} label="Avg Heart Rate" unit="bpm" explanation="Your average heart rate today." loading={loading} size="md" />
          </div>
          <div className="ai-readout">
            <p className="text-[11px] text-white/70 leading-relaxed">
              {loading ? "Calculating..." : (
                recoveryScore == null ? "Connect your WHOOP for a strain target." :
                recoveryScore >= 66 ? "🟢 Recovered. Target 14-18 strain. Push into zones 3-4." :
                recoveryScore >= 33 ? "🟡 Moderate. Target 10-14 strain. Stay in zones 2-3." :
                "🔴 Low recovery. Target 0-8 strain. Light movement only."
              )}
            </p>
          </div>
          <StrainZones workouts={recentWorkouts} explanation="Heart rate zones show where your training time is spent." loading={loading} />
          <div>
            <p className="section-title">Recent Workouts</p>
            <div className="space-y-3 mt-3">
              {loading ? (
                <div className="card-grid--2col">
                  {[...Array(4)].map((_, i) => <div key={i} className="skeleton skeleton--card" style={{ height: 160 }} />)}
                </div>
                ...
                <div className="card-grid--2col">
                  {recentWorkouts.map((w) => (
                    <WorkoutCard key={w.id} workout={w} explanation={`${getSportName(w.sport_id)} • ${new Date(w.start).toLocaleDateString("en-US", { weekday: "short", month: "short", day: "numeric" })}`} loading={false} />
                  ))}
                </div>
              )}
            </div>
      ...
      {tab === "trends" && (
        <div className="space-y-3">
          <TrendChart data={recoveryTrend as unknown as Array<Record<string, unknown>>} dataKey="value" period="30d" label="Recovery" explanation="Your recovery score over time." color="#34d399" loading={loading} />
          <TrendChart data={hrvTrend as unknown as Array<Record<string, unknown>>} dataKey="value" period="30d" label="HRV" unit="ms" explanation="Heart Rate Variability trend." color="#00d4ff" loading={loading} />
          <TrendChart data={rhrTrend as unknown as Array<Record<string, unknown>>} dataKey="value" period="30d" label="Resting HR" unit="bpm" explanation="Resting heart rate trend." color="#ff2d55" loading={loading} />
          <TrendChart data={sleepTrend as unknown as Array<Record<string, unknown>>} dataKey="value" period="30d" label="Sleep Performance" explanation="How well you're meeting your sleep need." color="#a78bfa" loading={loading} />
          <div className="glass p-4">
